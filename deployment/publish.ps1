...
$AppPublishPath = Join-Path $RootPath "Rex\bin\Publish"
...
$TempPayload = Join-Path $env:TEMP "AppPayload_$(Get-Random)"
...
Copy-Item -Path "$AppPublishPath\*" -Destination $TempPayload -Recurse
...
try {
    $AppDll = Join-Path $AppPublishPath "Rex.dll"
